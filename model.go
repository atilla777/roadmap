...
func scanPhase(row *sql.Row) (Phase, error) {
	var p Phase
	err := row.Scan(&p.ID, &p.ProjectID, &p.Title, &p.SortOrder, &p.CreatedAt)
	return p, err
}
...
func scanPhases(rows *sql.Rows) ([]Phase, error) {
	...
	for rows.Next() {
		var p Phase
		if err := rows.Scan(&p.ID, &p.ProjectID, &p.Title, &p.SortOrder, &p.CreatedAt); err != nil {
			return nil, err
		}
...
func scanTask(row *sql.Row) (Task, error) {
	...
	err := row.Scan(&t.ID, &t.ProjectID, &t.Title, &t.Status, &t.PhaseID, &t.SortOrder, &t.CreatedAt, &t.UpdatedAt, &phaseTitle)
	if phaseTitle.Valid {
...
func scanTasks(rows *sql.Rows) ([]Task, error) {
	...
	for rows.Next() {
		var t Task
		var phaseTitle sql.NullString
		if err := rows.Scan(&t.ID, &t.ProjectID, &t.Title, &t.Status, &t.PhaseID, &t.SortOrder, &t.CreatedAt, &t.UpdatedAt, &phaseTitle); err != nil {
			return nil, err
		}
		if phaseTitle.Valid {
			t.PhaseTitle = phaseTitle.String
...
const taskSelectCols = `t.id, t.project_id, t.title, t.status, t.phase_id, t.sort_order, t.created_at, t.updated_at, p.title`
